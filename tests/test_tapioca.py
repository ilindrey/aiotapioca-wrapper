...
async def test_is_pickleable(mocked):
    ...
    mocked.get(pickle_client.test().data,
...
async def test_delete_request(mocked, client):
    mocked.delete(client.test().data,
                  body='{"data": {"key": "value"}}',
                  status=201,
                  content_type='application/json')
...
async def test_token_expired_automatically_refresh_authentication(mocked, token_refresh_client):
    mocked.post(
        token_refresh_client.test().data,
        callback=callback_401,
        content_type="application/json",
...
async def test_stores_refresh_authentication_method_response_for_further_access(mocked, token_refresh_client):
    mocked.post(
        token_refresh_client.test().data,
        callback=callback_401,
        content_type="application/json",
...
async def test_raises_error_if_refresh_authentication_method_returns_falsy_value(mocked):
    async with FailTokenRefreshClient(token='token', refresh_token_by_default=True) as fail_client:
...
async def test_xml_post_string(mocked, xml_client):
    mocked.post(xml_client.test().data,
               body='Any response',
               status=200,
               content_type='application/json'
                  )
    ...
    request_body = mocked.requests[('POST', URL(xml_client.test().data))][0].kwargs['data']
